% This is a test file for TeX2CNL.
% It is an edited version of a chapter from Dense Sphere Packings.

%publications-of-thomas-hales/geometry/dense-sphere-packings/packing.tex 
 
% TeX2CNL version started July 20 2019
% Thomas Hales

% The character set used in this file is rather small
% cat test_packing.tex | tr -d '[a-zA-Z0-9%$<>^{}()_+-&@,.:;=~/!]' | tr -d '\r\n\\ `\*|#'

% Issues function calls f(a,b) vs curry f a b 
...
\CnlCustom[1]\newterm{ #1 }
\CnlDelete[2]\indy
\CnlDelete[2]\formaldef
\CnlDelete[1]\guid
\CnlDelete[1]\chapter
\CnlDelete\figDEQCVQL % fig:saturated
\CnlDelete\figXOHAZWO % fig:voronoi
\CnlDelete\figKFETCJS % fig:vset
\CnlDelete\figHFFTUNW % fig:rogers-omega
\CnlDelete\figBUGZBTW % fig:rogers-random 
\CnlDelete\figELMXAFH % fig:rogers-ill-defined
\CnlDelete\figNOCHOTB % fig:rogers-sqrt2
\CnlDelete\figYAJOTSL % fig:rogers-fact
\CnlDelete\figKVIVUOT % fig:marchal-variety
\CnlDelete\figBWEYURN % fig:marchal-2d
\CnlDelete\figFIFJALK % fig:ferguson-hales
\CnlDelete\figTULIGLY % fig:M
\CnlDelete\figBJLIEKB % fig:L
\CnlDelete\figPQFEXQN % fig:fg1
\CnlDelete\figJXEHXQY % fig:fg
\CnlDelete\figYAHDBVO % fig:reference-w
\CnlDelete\figZXEVDCA % fig:marchal-polyhedron
\CnlCustom[1]\nsqrt{ \sqrt{#1} }
\CnlNoExpand\vecR
\def\vecR#1{\ring{R}^{#1}}
\CnlNoExpand\Real
...
\CnlNoExpand\pi
\CnlNoExpand\ups
\CnlNoExpand\openBall  % B(p,r) in TeX
\def\openBall#1#2{B(#1,#2)}
\CnlNoExpand\floor
...
A \newterm{packing} $ V\subset \vecR{3}$ is a set such that
\[ 
\forall  \u,~\v\in  V.~  \norm{ \u}{\v} < 2 \Rightarrow ( \u=\v).
\]  
A set $V$ is \newterm{saturated} if for every $\p\in\vecR{3}$ there
...
is measurable with measure $4\pi r^3/3$.
 Set $ V(\p,r) = V \cap
\openBall{\p}{r}$. %and $ V^*(\p,r) = V(\p,r)\setminus \{\p\}$.
\formaldef{ball}{ball}
\formaldef{$V(\p,r)$}{V INTER ball(p,r)}
\indy{Index}{measure}%
\indy{Notation}{B@$\openBall{\v}{r}$ (open ball)}%
\indy{Notation}{V1@$V(\p,r)=V\cap \openBall{\p}{r}$}%
...
\] 
It is a one-to-one map from $ V(\p,r)$ into the set $\vecZ{3}\cap
...
%\cap \openBall{\v}{t}$ be the truncated Voronoi cell at radius $t$.
\end{definition}
...
\vecR{3} = \bigcup \{\Omega(V,\v)\mid \v \in V\}.
\end{equation}
\end{lemma}
...
\formaldef{$A$}{bis}
\formaldef{$\Aplus$}{bis\_le}
\begin{align*} 
\AA{\u}{\v} &= \{\p\in\vecR{3}
\mid 2(\v-\u)\cdot \p = \normo{\v}^2 - \normo{\u}^2 \},\\
\Aplus(\u,\v) &= \{\p\in\vecR{3}
\mid 2(\v-\u)\cdot \p \le \normo{\v}^2 - \normo{\u}^2 \},
\end{align*}
when $\u,\v\in\vecR{3}$.  The plane $\AA{\u}{\v}$ is the \newterm{bisector} of
$\{\u,\v\}$ and $\Aplus(\u,\v)$ is the \newterm{half-space} of points at least as
close to $\u$ as to $\v$.  
\end{definition}
\indy{Notation}{A1@$\AA{\u}{\v}$ (bisector)}%
\indy{Notation}{A2@$\Aplus(\u,\v)$ (half-space)}%
...
\begin{lemma}[Voronoi polyhedron]\guid{RHWVGNP}\label{lemma:V4} 
  ...
  $\Aplus(\v,\u)$ for $\u\in V(\v,4)\setminus\{\v\}$.
\end{lemma}
...
$\op{conv}\{\v,\p'\}$ for some $\p'\in\Omega(V,\v)\subset \openBall{\v}{2}$.
...
\] 
Choose $\u\in V\setminus V(\v,4)$ such that $\q\in \Aplus(\u,\v)$.  By the
triangle inequality,
\[  
\norm{\u}{\v} \le \norm{\u}{\q} + \norm{\v}{\q} \le 2\norm{\v}{\q} < 4.
\] 
...
The number of half-spaces $\Aplus(\v,\u)$ for $\u\in V(\v,4)$ is finite by
...
function $G: V\to\Real$ for a saturated packing $ V$, then there
...
\begin{equation} 
\op{vol}\, \openBall{ V}{\orz,r} \le \card( V(\orz,r+1))\, 4\pi/3.
\label{eqn:Abound}
\end{equation}
...
\newterm{Kepler conjecture} is to prove the bound $\delta(
...
$\Omega(V,\bu)$ in $\vecR{3}$, when $\bu\in \bV(k)$; it is not the
...
Let $V\subset\vecR{3}$ be a saturated packing.
...
\begin{lemma}[facets]\guid{IDBEZAL}\label{lemma:omega-facet} 
  Let $V\subset\vecR{3}$ be a saturated packing.  Let $\bu\in
...
\begin{definition}[$\omega$]\guid{JJGTQMN}\label{def:omega}
  ...
  $\omega_j=\omega_j(V,\bu)\in\vecR{3}$ by recursion over $j\le k$
...
%Define $\omega$ \mid \coprod_{j=0}^3 \bV(j)\to \vecR{3}$ by recursion over $j$ as follows.
%Let \[  
%\omega([\u]) = \u,
%\]  and
...
\formaldef{$R$}{rogers}
Let $V\subset\vecR{3}$ be a saturated packing. For $\bu\in \bV(k)$, let 
\[
...
Consequently,
\[ 
\vecR{3} = \bigcup\, \{ R(\bv) \mid \bv\in\bV(3)\}.
\] 
\end{lemma}
...
%\[ \vecR{3} = \bigcup\, \{\Omega(V, \bu)\mid \bu\in
%\bV(0)\}.\] 
...
% properly decreasing chain of affine sets: $\vecR{3}$, the bisector
...
\begin{definition}[circumcenter,~circumradius]\guid{IFLFHKT} 
  ...
  Let $S\subset\vecR{n}$.  A point $\p$ is a \newterm{circumcenter}
...
\end{definition}
\indy{Notation}{S@$S$ (finite subset of $\vecR{3}$)}%
...
\begin{lemma}[linear systems]\guid{QXSKIIT}\label{lemma:affine-system} 
  Let $S=\{\v_0,\ldots,\v_m\}\subset \vecR{n}$ be an affinely
...
\claim{A solution exists; that is, the image is all of $\,\vecR{m}$.}
...
$\p = \sum u_i \w_i\in W$ and let $\q\in\vecR{m}$ be the image of
$\p\in W$.  Then
\[  
...
\begin{lemma}[circumcenter exists]\guid{OAPVION} 
  Let $S\subset \vecR{n}$ be a nonempty affinely independent set.
...
\begin{proof}  The proof is by induction on $k$.  
 ...
 ball centered at $\u_0$, so its affine hull is $\vecR{3}$.  This is the
  ...
  $\dimaff\vecR{3} = 3$, $\dimaff\{\u_0\}=0$, $\vecR{3}\cap
  \{\u_0\} = \{\u_0\}$, and $\vecR{3}\perp \{\orz\}$.
...
\begin{lemma}[nondegeneracy]\guid{XYOFCGX}\label{lemma:sqrt2-close} 
  Let $V\subset\vecR{3}$ be a saturated packing.  Let $S\subset V$
...
\indy{Notation}{C@$C(S)$ (cell-like subset of $\vecR{3}$)}%
...
The set $C(\emptyset)$ is $\vecR{3}$.  The set $C(\{\w\})$ is a ball
...
that for all  $r\ge1$
\[
\sum_{X\subset \openBall{\orz}{r}} \gamma(X,f) \ge c_0 r^2,
\]
...
constant $c$ such that for all $r\ge 1$:% and all $\p\in\vecR{3}$:
\begin{align}\label{eqn:neg} 
-\sum G( \u,f) &= 
\sum \op{vol}(\Omega(V, \u)) 
-\sum 8m_1 + 
\sum \sum_{\v\in V\setminus\{\u\} } 8 m_2 f(h([\u;\v]))\notag \\
&\ge \sum_{X\subset \openBall{\orz}{r}} \gamma(X,f)  + c r^2,
\end{align}
...
\ge \sum_{X\subset \openBall{\orz}{r}} \op{vol}(X) + c_1 r^2 = T_1 + c_1 r^2.
\] 
...
\begin{align*} 
\sum_{X\subset \openBall{\orz}{r}} \op{tsol}(X) &= 
\sum_{X\subset \openBall{\orz}{r}~~} \sum_{ \u\in V( X)} \sol(X, \u)\\
...
-\sum_{X\subset \openBall{\orz}{r}}\left(\frac{2m_1}\pi\right) 
...
\begin{alignat*}{6}
&\phantom{=}&&\sum_{X\subset \openBall{\orz}{r}} 
&&\sum_{~~\ee\in E(X)~~~~~} \dih(X,\ee)  f(h(\ee)) \vspace{3pt}\\
&=&&\sum_{\ee\subset \openBall{\orz}{r}} 
&&\sum_{~~X\mid \ee\in E(X)} \dih(X,\ee)  f(h(\ee)) +O(r^2)\vspace{3pt}\\
&=&&\sum_{\ee\subset \openBall{\orz}{r}} &&2\pi f(h(\ee)) + O(r^2)\vspace{3pt} \\
&=&&\sum_{ \u_0\in  V(\orz,r)} 
...
\sum_{X\subset \openBall{\orz}{r}~~}\sum_{\ee\in E(X)}\dih(X,\ee) f(h(\ee)) + O(r^2) \\
...
\[
\sum_{X\subset \openBall{\orz}{r}} \gamma(X,L) \ge c_0 r^2.
\]
...
\begin{alignat*}{8}
\sum_{X\subset \openBall{\orz}{r}} \gamma(X,L) 
&=&&\sum_{X\subset \openBall{\orz}{r}\mid \op{EC}(X)\ne\emptyset} \hspace{-2em}\gamma(X,L) \quad +
\sum_{X\subset \openBall{\orz}{r}\mid \op{EC}(X)=\emptyset} \hspace{-2em}\gamma(X,L) \vspace{6pt}\\
&\ge &&\sum_{X\subset \openBall{\orz}{r}\mid \op{EC}(X)\ne\emptyset} \hspace{-2em}\gamma(X,L)\vspace{6pt} \\
&= &&\sum_{\hspace{1.8em}X\subset \openBall{\orz}{r}\hspace{1.8em}}\hspace{-2em}\gamma(X,L)\sum_{\ee \in \op{EC}(X)}\op{wt}(X) \vspace{6pt}\\
&= &&\sum_{\hspace{1.85em}\ee\subset \openBall{\orz}{r}\hspace{1.85em}}\hspace{-2.5em}\sum_{~~~~X\!\mid\! \ee \in \op{EC}(X)}\hspace{-1.1em}\gamma(X,L)\op{wt}(X) &+ O(r^2)\vspace{6pt}\\
&= &&\sum_{\hspace{1.85em}\ee\subset \openBall{\orz}{r}\hspace{1.85em}}\hspace{-1em}\Gamma(\ee) &+ O(r^2)\vspace{6pt}\\
&\ge&& ~~O(r^2).
\end{alignat*}
\end{proof}
...
\begin{definition}[$\BB$]\guid{WTKURHK} 
  ...
  $\bar \openBall{\orz}{2h_0}\setminus \openBall{\orz}{2}$, where $\bar \openBall{\orz}{r}$ is
...
\begin{lemma}[]\guid{GOTCJAH}\label{lemma:ngon} 
  Let $P$ be a bounded polyhedron in $\vecR{3}$ that contains $\orz$
...
set $ V\subset\vecR{3}\setminus \openBall{\orz}{2}$ to be \newterm{weakly
  saturated} with parameters $(r,r')$ if for every $\p\in\vecR{3}$
\[  
