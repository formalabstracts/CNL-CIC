...
% Issues function calls f(a,b) vs curry f a b \app{f}{(a,b)}
% \vspace*{250px} how to get at it? - For now, documents should use alternatives, such as \null ..\vspace{...}.
% \hspace*{}
% \left. (with period, makes an invisible delimiter)
% \linebreak[number]
% \predisplaypenalty=0 and assignments.

% What about \hbox{}? We might want two variants,
...
% prohibited control sequences. They should be used outside the cnl environment.
% We should include most TeX primitive control sequences here.
\CnlError\if
\CnlError\else
\CnlError\fi
\CnlError\let
...
%% Customize for this file.

\CnlCustom[1]\newterm{ #1 }
\CnlDelete[2]\indy
\CnlDelete[2]\formaldef
\CnlDelete[1]\formalauthor
\CnlDelete[1]\guid
\CnlDelete[1]\chapter
\CnlDelete\figDEQCVQL 
\CnlDelete\figXOHAZWO 
\CnlDelete\figKFETCJS 
\CnlDelete\figHFFTUNW 
\CnlDelete\figBUGZBTW 
\CnlDelete\figELMXAFH 
\CnlDelete\figNOCHOTB 
\CnlDelete\figYAJOTSL 
\CnlDelete\figKVIVUOT 
\CnlDelete\figBWEYURN 
\CnlDelete\figFIFJALK 
\CnlDelete\figTULIGLY 
\CnlDelete\figBJLIEKB 
\CnlDelete\figPQFEXQN 
\CnlDelete\figJXEHXQY 
\CnlDelete\figYAHDBVO 
\CnlDelete\figZXEVDCA 
\CnlCustom[1]\nsqrt{ \sqrt{#1} }
\CnlNoExpand\vecR
\def\vecR#1{\ring{R}^{#1}}
\CnlNoExpand\Real
...
%SUM
\def\sumfinitepred#1#2#3{{\sum_{#2} {#3}}} %#1 free
\CnlCustom[3]\sumfinitepred{\sumfinite{\setcomp{#1}{#2}}{\LAMBDA #1, #3}}
\def\sumupto#1#2#3#4{{\sum_{#1 = #2}^{#3} {#4}}}
\CnlCustom[4]\sumupto{\sumfinite{((#2) .. (#3))}{\LAMBDA #1, #4}}
\def\sumvar#1{#1\mid}
\CnlDelete[1]\sumvar
\def\andcomma{,}
\CnlCustom\andcomma{and}

%ELLIPSIS
\def\setenumdots#1#2#3{\{{#1}_{#2},\ldots,{#1}_{#3}\}}
\CnlCustom[3]\setenumdots{\image{#1}{((#2) .. (#3))}}
\CnlNoExpand\image
\def\setenumrange#1#2{\{#1,\ldots,#2\}}
\CnlCustom[2]\setenumrange{((#1) .. (#2))}
\def\listdots#1#2#3{[{#1}_{#2};\ldots;{#1}_{#3}]}
\CnlCustom[2]\listdots{\listmap{#1}{\listrange{#2}{#3}}}
%for $i=1,...,j$
\def\fordots#1#2#3{#1=#2,\ldots,#3}
\CnlCustom[3]\fordots{#1\in ((#2) .. (#3))}
...
Informally, a \newterm{packing} is an arrangement of congruent balls
...
\openBall{\p}{r}$. 
\formaldef{ball}{ball}
\formaldef{$\Vee{V}(\p,r)$}{V INTER ball(p,r)}
\indy{Index}{measure}%
\indy{Notation}{B@$\openBall{\v}{r}$ (open ball)}%
\indy{Notation}{V1@$\Vee{V}(\p,r)=V\cap \openBall{\p}{r}$}%
...
\begin{lemma}[Voronoi partition]\label{lemma:Voronoi-partition}
\preamble{
 \guid{TIWWFYQ}
}
...
\begin{lemma}[Voronoi polyhedron]
...
\begin{theorem*}[Kepler's conjecture on dense packings]
...
\begin{definition}[negligible,~FCC-compatible]
...
\begin{lemma}[reduction to finite dimensions]
...
% 
If there exists a \newterm{negligible}
\fullterm{FCC-compatible}{FCC!compatible} function $G: V\to\Real$ for
...
\preamble{
 ...
 \formaldef{$\trunc{\bu}{j}$} {truncate\_simplex}\hspace{-3pt}
}
%
...
\preamble{
 \guid{JJGTQMN}
 \formaldef{$\omega_k$} {omega\_list\_n} 
 \formaldef{$\omega$}{omega\_list} 
}
%
...
\claim{The point $\omega(\bu)$ exists when $\bu\in \Vee{V}(k)$.}  Indeed,
...
\begin{lemma}[Rogers decomposition]
...
\claim{We claim $N = \setenumrange{j}{3}$.}  Indeed, to see that $j\in N$, we note that
\[  
...
\claim{We claim that $s_i = t_i$, and $\sigma_{i+1} = \sumupto{j}{i+1}{3}{t_j}$, 
for $\fordots{i}{0}{k-1}$.}  Indeed, the
...
\begin{lemma}[circumcenter exists]
...
\claim{We claim that $A_{k+1}\cap C_{k+1} = \braced{\q_{k+1}}$.}  Indeed, by the
...
\begin{lemma}[Rogers simplex and circumcenter]
\label{lemma:v2} 
\preamble{
 \guid{XNHPWAB}
}
...
\item\claim{The set $\setcomp{\omega(\trunc{\bu}{j})}{ j\le k}$ has affine dimension $k$.}
%
%
...
\begin{lemma}[extreme point rearrangement]
\label{lemma:perm-Vk} 
\preamble{
 \guid{YIFVQDV}
}
...
\begin{lemma}[permutations one-to-one]
...
\begin{lemma}[coset representatives]
...
\begin{definition}[Marchal cells]
\label{def:mcell}
\preamble{
 \guid{QEEHXUB}
...
the same length.  One is an $i$-\newterm{rearrangement} of the other
...
\preamble{
 \guid{LEPJBDJ}
 \formaldef{$\CapV{V}(X)$}{VX}
 \indy{Notation}{V@$\CapV{V}(X)=V\cap X$}%
}
%
...
%
Let $E(X)$ be the set of \newterm{extremal edges} of the $k$-cell $X$
...
\begin{definition}[$\sol_0$,~$\tau_0$,~$m_1$,~$m_2$,~$h_+$,~$M$]
\label{def:sol0}
\preamble{
 \guid{AOZUTMU}
...
\preamble{
 \guid{KGFDCFM}
 \formaldef{$\gammaX$}{gammaX} 
 \indy{Notation}{zzc@$\gammaX$ (packing inequality)}%
}
%
...
\begin{definition}[cell cluster,~$\Gamma$]
\label{def:gammaL} 
\preamble{
 \guid{YSULGYR}
...
edge of a $k$-cell $X$ of $V$ for some $2\le k\le 4$.  A \newterm{cell
  cluster} is the set
\[  
...
\begin{theorem}[cell cluster inequality]
\label{lemma:cluster}
\preamble{
 \guid{OXLZLEZ} 
}
...
polyhedron is bounded.  By a \cc{WAZLDCD}{} %
