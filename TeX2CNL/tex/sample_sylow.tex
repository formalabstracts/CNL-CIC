...
\CnlCustom\fun{fun}
\CnlCustom[2]\funmapsto{(fun #1 := #2)}
%\funalign{A}&{\to}{B}\\{a}{\mapsto}{b}
\CnlCustom[6]\funalign{(fun (#4 : #1) := (#6 : #3))}
\CnlCustom\Prop{Prop}
\CnlCustom\Type{Type}
\CnlCustom\iff{iff}
\CnlCustom\optarg{}
...
\deflabel{set}
Let \df{set} (of)\ $\alpha$ be the type $\alpha \to\Prop$.
\end{definition}

\deflabel{universe}
Let \df{universe} of $\alpha$ stand for the function
\[
\funmapsto{(\blank : \alpha)}{\h{true}}.
\]
\end{definition}

\deflabel{empty set}
Let empty set (inferring $\alpha$)
stand for the function
\[
\funmapsto{(\blank\ : \alpha)}{\h{false}}.
\]
\end{definition}

Let $\emptyset$ stand for empty set.
...
\deflabel{in}
Let $x \in X$ (inferring C :
\h{notation-in}) denote $C\h{.notation-in}\ x\ X$.
\end{definition}
...
Let $X\subset Y$ (inferring $C : \h{notation-subset})$) denote
...
By recording it as an equivalence, definitions involving
...
We say that $\alpha$ is a \prefix{type~}\df{finite} type iff the
universe of $\alpha$ is finite.

\begin{definition}
Assume that $\alpha$ is a \prefix{type~}finite type.  The
\prefix{type~}\df{size} of $\alpha$ is the size of the universe of
$\alpha$.  This exists and is unique.
\end{definition}

\begin{definition}
Assume that $\alpha$ is a finite type.  The \prefix{type~}\df{support}
...
Let $x \le y$ (inferring
$C:\h{notation-le}$) denote $C\h{.notation-le}\ x\ y$.
...
\deflabel{nat le}
  We say that $\h{nat.nat-le}\ m \ n$ iff there exists $d$ such that $m + d = n$.
\end{definition}
...
An isomorphism from $F$ to $G$ of complete ordered fields  is a bijection
\[
f:F\to G
\]
such that 
...
For all complete ordered fields $F,\ G$,
we record a canonical isomorphism $f : F \to G :=$ 
the isomorphism from $F$ to $G$ of complete ordered fields.
This exists and is unique.
...
\begin{remark}
For all complete ordered fields $F,\ G$, there exists a unique
