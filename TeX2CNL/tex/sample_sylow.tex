...
\deflabel{list}
Let $\h{list}\ \{\alpha\}$ be the inductive type
\begin{envMatch}
$\h{null}$ &$:$& $\h{list}$\\
...
\[
\h{list.cons}\ \{\alpha\}\ x\ X.
\]
\end{definition}

\deflabel{length}
Let $\h{list.length}\ \{\alpha\}\ : \h{list}\to \Nat$ = \par\function
\begin{envMatch}
$[]$ &$\assign$& $0$\\
...
\deflabel{multiset} Let \h{multiset}\ $\{\alpha\}$ denote the quotient
of \h{list}\ $\{\alpha\}$ by the equivalence relation
...
\h{over}\ \alpha) : \h{set over}\ \alpha \\ \assign \{ x \mid
x \in \h{support of}\ X \}.
\end{align*}
\end{definition}
...
for every $(x\ y),$ there exists a least upper bound of $\{x,y\}$ in
$R$.
\end{definition}

\deflabel{meet semilattice} We say $R$ is a \df{meet semilattice} iff
for every $x, y$ there exists a greatest lower bound of $\{x,y\}$ in
$R$.
\end{definition}

\deflabel{join} Assume $R$ is a join semilattice.  Let
$\h{\df{join}}\ x\ y$ be the least upper bound of $\{x,y\}$ in $R$.
This exists and is unique.
\end{definition}

\deflabel{meet} Assume $R$ is a meet semilattice.  Let
$\h{\df{meet}}\ x\ y$ be the greatest lower bound of $\{x,y\}$ in $R$.
This exists and is unique.
\end{definition}
...
\end{itemize}
\where\ $\{A \assign \h{absolute-value}\}$.
\end{definition}
