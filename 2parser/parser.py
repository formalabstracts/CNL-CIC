...
class Parse:
    ...
    def probe(self):
        ...
        def f(item):
            try:
                ...
                raise ParseError(pe + [ErrorItem(item=item,production='',nonterminal='probe')])
    ...
    def reparse(self):
        ...
        def f(item):
            try:
                ...
                if not acc:
                    return item
                item1 = token.Item(acc,0,None,[])
                item2 = (self + Parse.finished()).process(item1)
                item3 = token.update(item2.acc,item)
                return item3
            except ParseError as pe:
                raise ParseError(pe + [ErrorItem(item=item,production='',nonterminal='reparse')])
        return Parse(f,self.nonterminal,self.production)
    ...
    def reparse_list(self):
        ...
        def f(item):
            try:
                acc = item.acc
                its1 = [token.Item(a,0,None,[]) for a in acc]
                acc2 = [(self + Parse.finished()).process(it).acc for it in its1]
                item3 = token.update(acc2,item)
                return item3
            except ParseError as pe:
                raise ParseError(pe + [ErrorItem(item=item,production='',nonterminal='reparse_list')])
        return Parse(f,self.nonterminal,self.production)

    def __add__(self,other):
        """combine two parsers in succession, returning pair of results."""
        def f(item:token.Item):
            item1 = self.process(item)
            acc1 = item1.acc
            item2 = other.process(token.update(None,item1))
            return token.update((acc1,item2.acc),item2)
        return Parse(f,'_+_')
            ...
            try:
                ...
                try:
                    ...
                    if item2.pos > item1.pos: #raise the most progressed
                        raise ParseError(pe2)
                    raise ParseError(pe1)
        return Parse(f,'_|_')
    ...
    def nocatch(self,msg=''): 
        ...
        def f(item):
            try:
                ...
                raise ParseNoCatch(pe+[ErrorItem(item=item,nonterminal=nonterminal,production='')])
    ...
    def treat(self,treatment,msg=''):
        ...
        def f(item):
            item1 = self.process(item)
            try:
                item2 = token.update(treatment(item1.acc),item1)
                return item2
            except ParseError as pe:
                nonterminal=msg.setdefault(msg,'treat')
                raise ParseError(pe+[ErrorItem(item=item,nonterminal=nonterminal,production='')])
    ...
    def many(self):
        ...
        def f(item):
            try:
                ...
                return item
            acc1 = item1.acc
            item2 = self.many().process(token.update(None,item1)) #this doesn't fail
            return token.update([acc1]+item2.acc,item2)  
        return Parse(f,'*')
    ...
    def separated_nonempty_list(self,sep):
        ...
        return (self + 
                (sep + self).treat(lib.snd).many()).treat(lib.prepend).name('list'+sep)
              
    def separated_list(self,sep):
        """sequence of parses with separation sep"""
        return (self.separated_nonempty_list(sep) | Parse.identity().nil()).name('list'+sep)
    ...
    def if_type(self,ts): 
        ...
        return self.if_test(p).name(self.ts.repr())
    ...
    def all(prs):
        ...
        def f(item):
            if not prs:
                return token.update([],item)
            else:
                item1 = prs[0].process(item)
                acc1 = item1.acc
                item2 = Parse.all(prs[1:]).process(token.update(None,item1))
                return token.update([acc1]+item2.acc,item2)
        return Parse(f,'all')
...
class Inner():
    ...
    def escape():
        def f(acc):
            return ('escape',getvalue(acc))
        return next_type('ESCAPED').treat(f).name('escape')
    ...
    def bracket():
        def f(acc):
            ((_,a),_) = acc
            return ('bracket',a)
        return (next_type('[') + Inner.annotated() + next_type(']')).name('bracket')
    
    def opt():
        def f(acc):
            ((_,a),_) = acc
            return ('opt',a)
        return (next_type('(') + Inner.annotated().separated_nonempty_list('|') + next_type(')')).name('opt')
    ...
    def ending():
        def f(acc):
            ...
            if a:
                l.append(a)
            if b:
                l.append('rep',b.value)
            return l
        return (Inner.bracket().possibly() + ((next_type('*') or next_type('?')).possibly())).name('ending')
    
    def parser():
        return ((Inner.id() or Inner.dot() or Inner.escape() or Inner.opt()) + Inner.ending()).name('parser')

    def annotated():
        def f(acc):
            (a,b) = acc
            return {'label':getvalue(a),'parser':b}
        return (Inner.label().possibly() + Inner.parser()).treat(f)
    
    def top_level():
        def f(acc):
            (((a,b),c),p) = acc
            return {'finished':bool(p),'nonterminal':a,'production':b,'annotated':c}
        return ((Inner.id() + next_value('/')).treat(lib.fst) + Inner.id() + Inner.annotated().plus() + next_value('$').possibly()).treat(f)
    

    
st = "a/b th:\the$"
inner_lexer.tokenizer.input(st)
for t in inner_lexer.tokenizer:
    print(t)
inner_lexer.tokenizer.input(st)
it = token.init_item(list(inner_lexer.tokenizer))
itout = Inner.top_level().process(it)
print(itout)
